    ...
    Just info -> do
      let battPctNum :: Int
          ...
          tpl' = setManyAttrib [ ("percentage", show battPctNum)
                               , ("time", battTime)
                               , ("charge", if batteryOn then "<span fgcolor='#00FF00'>+</span>"
                                                         else "<span fgcolor='#FF0000'>-</span>")
                               ] tpl
      return $ render tpl'
...
defaultBatteryConfig = defaultBarConfig colorFunc
   where
     colorFunc pct
       | pct < 0.1 = (1, 0, 0)
       | pct < 0.9 = (0.5, 0.5, 0.5)
...
batteryBarNew :: BarConfig -- ^ Configuration options for the bar display
                 -> String -- ^ Configuration for text display
                 -> Double -- ^ Polling period in seconds
                 -> IO Widget
batteryBarNew battCfg battPattern pollSeconds = do
  battCtxt <- batteryContextNew
  case battCtxt of
    Nothing -> labelNew (Just "No battery") >>= return . toWidget
    Just ctxt -> do
      ...
      b <- hBoxNew False 1
      txt <- textBatteryNew battPattern pollSeconds
      r <- newIORef ctxt
